...
class PoseVisualizationPage(DialogWithGuide, HandleErrorMessage):
    ...
    def setValues(self, inserted_id):
        try:
            ...
            self.nameLineEdit.setText(patient_data["patient_name"])
            self.timeLineEdit.setText(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(vis_data["time"] / 1e9)))
            pixmap = QPixmap()
            print(vis_data["visualization"])
            pixmap.loadFromData(QByteArray(base64.b64decode(vis_data["visualization"]["$binary"]['base64'])))
    ...
    def save(self, inserted_id):
        remark = self.lineEdit.toPlainText()
        if remark:
            query_data = {
                "_id": inserted_id,
            }
